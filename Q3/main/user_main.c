...
void app_main(void)
{
	...
	xTaskCreate(GPIOStatusMsg, "Task1", 2048, NULL, 10, NULL);
	...
	if (Semaphore != NULL)
	{
		...
		xTaskCreate(GPIOPinHigh, "Task2", 2048, NULL, 8, NULL);
		
		//Creating task to toggle the GPIO pin to logic low
		xTaskCreate(GPIOPinLow, "Task3", 2048, NULL, 9, NULL);
	}
...
void vApplicationIdleHook( void )
{
	//Putting the ESP8266 into a light sleep for 500ms
	ESP_LOGI(TAG, "Entering into Light Sleep for 1500ms\n");

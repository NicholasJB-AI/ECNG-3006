...
void app_main(void)
{
	...
	//Assigning the same priority to GPIOStatusMsg
	...
	if (Semaphore != NULL)
	{
		...
		//Assigning the same priority to GPIOPinHigh
		xTaskCreate(GPIOPinHigh, "Task2", 2048, NULL, 8, NULL);
		
		//Creating task to toggle the GPIO pin to logic low
		//Assigning the same priority to GPIOPinLow
		xTaskCreate(GPIOPinLow, "Task3", 2048, NULL, 8, NULL);
	}
